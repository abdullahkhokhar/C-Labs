...
int truncate(char* s, int n){
  // length of s, does NOT include the null terminating character
  int string_len = strlen(s);
  if(string_len <= n){
    return 0;
  }

  // we know string has more than n characters
  // the difference
  int difference = string_len - n;
  // truncate
  s[n] = '\0'; // string has greater than n characters so can do this
  return difference;
